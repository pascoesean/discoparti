...
get_pca_loadings <- function(filename, cell_type) {
  data <- schard::h5ad2seurat(paste(getwd(), filename, sep=""))

  ss <- subset(data, subset = cell_type_0.60 %in% c(cell_type))
  ...
  return(top_genes_long)
}

# plot the pca loadings we just got with `get_pca_loadings()`
plot_pca_loadings <- function(loadings) {
  ggplot(loadings, aes(x = reorder(gene, loading), y = loading, fill = PC)) +
